...
public class ABC {
    ...
    public boolean asignarPedidoPoblacionInicial(PedidoModel pedido, int opcion) {
        ...
        for (int a = 0; a < rutas.size(); a++) {
            ...
            if(opcion == 0){
                if(rutas.get(a).getHorasDeLlegada().get(rutas.get(a).getHorasDeLlegada().size()-1) > inicio.atZone(zoneId).toEpochSecond()){
        ...
        if (!bool) {
            ...
            if(true){
                ...
                for (int i = 0; i < oficinas.size(); i++) {
                    if (i == 0) {
                        horasLlegadaLong.add(rutas.get(iMenor).getHorasDeLlegada().get(rutas.get(iMenor).getHorasDeLlegada().size()));
                        horasLlegada.add(LocalDateTime.ofInstant(Instant.ofEpochSecond(rutas.get(iMenor).getHorasDeLlegada().get(rutas.get(iMenor).getHorasDeLlegada().size())), zoneId));
                    } else {
        ...
        for (int i = 0; i < vehiculos.size(); i++) {
            ...
            if (vehiculos.get(i).getEstado() == EstadoUnidadTransporte.DISPONIBLE) {
                ...
                if (vehiculos.get(i).getCapacidadDisponible() >= pedido.getCantPaquetesNoAsignado()) {
                    ...
                    // Asignación Ruta
                    Long idUnidadTransporte = vehiculos.get(i).getId();
                    PedidoParcialModel pedidoParcial = new PedidoParcialModel(0L, pedido.getId(), -1, vehiculos.get(i).getCapacidadDisponible(), 0L, idRuta);
                    pedidosParciales.add(pedidoParcial);
                    // Pedido
    ...
    public boolean asignarPedidoRutaVehiculo(PedidoModel pedido, Ruta ruta, int opcion) {
        ArrayList<UnidadTransporteModel> vehiculos;
        if(opcion == 0) vehiculos = Mapa.vehiculosSimulacion;
        else vehiculos = Mapa.vehiculosDiaDia;
        ...
        if (!encontrado) {
            ...
            if (vehiculos.get(Math.toIntExact(ruta.getIdUnidadTransporte())).getCapacidadDisponible() > pedido.getCantPaquetesNoAsignado()) {
    ...
    ArrayList<UnidadTransporteModel> actualizarMantenimiento(List<PedidoParcialModel> auxPedidos, int opcion) {
        ...
        for (int i = 0; i < auxPedidos.size(); i++) {
            for (int j = 0; j < vehiculos.size(); j++) {
                if (vehiculos.get(j).getEstado() == EstadoUnidadTransporte.DISPONIBLE || vehiculos.get(j).getEstado() == EstadoUnidadTransporte.EN_TRANSITO) {
                    ...
                    LocalDateTime auxd2 = vehiculos.get(j).getFechaMantenimiento().toInstant().atZone(ZoneId.systemDefault()).toLocalDateTime();
                    if (auxd.isBefore(auxd2.plusDays(1))) {
                        vehiculos.get(j).setEstado(EstadoUnidadTransporte.EN_MANTENIMIENTO);
                        if(opcion == 0) Mapa.vehiculosSimulacion.get(j).setEstado(EstadoUnidadTransporte.EN_MANTENIMIENTO);
                        else Mapa.vehiculosDiaDia.get(j).setEstado(EstadoUnidadTransporte.EN_MANTENIMIENTO);
                        auxArrayVehiculos.add(vehiculos.get(j));
                    }
                }
            }
        }
